 ...
 */
	...
	$message_file=((IsSet($_SERVER['argv']) && count($_SERVER['argv'])>1) ? $_SERVER['argv'][1] : 'Appointment.eml');
 ...
 */
	...
	//INSERTS THE PARSED DATA INTO THE TABLE
	for($message = 0; $message < count($decoded); $message++){
	 	$from=$decoded[$message]['Headers']['from:'];
	 	$to=$decoded[$message]['Headers']['to:'];
	 	$subject=$decoded[$message]['Headers']['subject:'];
	 	$date=$decoded[$message]['Headers']['date:'];
		$HTMLBody=$decoded[$message]['Body'];
	
		
		if($mime->Analyze($decoded[$message], $results)){
			$body=$results['Alternative'][0]['Data'];
			$Attachment=$results['Attachments'][0]['Data'];
		}
		else{
			echo 'MIME message analyse error: '.$mime->error."\n";
		}
		...
		try {
			//Many of these column names are keywords, need to use backtick
			$sql = "INSERT INTO messages (`from:`,`to:`,`subject:`,`date:`,`body:`,`HTMLBody:`,`Attachment:`)
			VALUES (?,?,?,?,?,?,?)";
		
			//Without the prepare statement, Strings containing apostrophe(') will cause syntax errors in VALUES
			$stmt= $pdo->prepare($sql);
			$stmt->execute([$from, $to, $subject, $date, $body, $HTMLBody, $Attachment]);
...
//Prints whole array********************************************************************
	...
	foreach($decoded as $a=>$b){
		...
		//EACH EMAIL STRUCTURE(HEADER, PARTS, POSITION, BODYPART,BODYLENGTH,EXTRACTED-ADDRESS)
		foreach($b as $c=>$d){
		echo "**FIELD**: ".$c;
		echo "<br/>";
				
				//$e is each Substructure(HEADER->return-path,to,subject,from,reply-to,date)
				//$f Information of each substructure
				foreach($d as $e=>$f){
					echo $e;
					echo "********  ";
					echo $f;
